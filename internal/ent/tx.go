...
func (tx *Tx) init() {
	...
	tx.LearningStreak = NewLearningStreakClient(tx.config)
	tx.PronunciationAttempt = NewPronunciationAttemptClient(tx.config)
	tx.Question = NewQuestionClient(tx.config)
	tx.QuestionAttempt = NewQuestionAttemptClient(tx.config)
	tx.QuestionReview = NewQuestionReviewClient(tx.config)
...
// applies a query, for example: Deck.QueryXXX(), the query will be executed
